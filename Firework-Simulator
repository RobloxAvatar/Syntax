...
do
	local Window = library:AddWindow("Firework Simulator", {
		main_color = Color3.fromRGB(13, 105, 172),
    ...
    do
        ...
        local switch = MainTab:AddSwitch("Infinite Money", function(bool)
	        _G.InfMoney = bool
            if bool == true then
                local ohNumber1 = 10000000000000
                workspace.AddMoneyD:FireServer(ohNumber1)
            end
        end)

        local switch2 = MainTab:AddSwitch("Drop All Firework", function(bool)
	        _G.AutoDrop = bool
        end)

        MainTab:AddButton("Infinite Rebirths", function()
            local ohNumber2 = 10000000000000
            workspace.AddRebirth:FireServer(ohNumber2)
        end)

        _G.Item = ""

        local dropdown = ItemsTab:AddDropdown("Item", function(item)
            if item == "DumBum 2G" then
                _G.Item = item
	        end
        end)
        ...
        if _G.AutoDrop then
            pcall(function()
                for i,v in pairs(game:GetService("Players").LocalPlayer.Backpack:GetChildren()) do
                    if v.Name ~= "Lighter" and v.Name ~= "LocalScript" and _G.AutoDrop == true then
                        game:GetService("Players").LocalPlayer.Character.Humanoid:EquipTool(v)
                        local ohString1 = "Create"
                        game.Players.LocalPlayer.Character[v.Name].Mouse:FireServer(ohString1)
                        wait(0.1)
                    end
                end
            end)
...
spawn(function()
    while task.wait(0.1) do
        if _G.AutoBuy then
            pcall(function()
                if _G.Item == "DumBum 2G" then
                    local ohNumber1 = 21
                    game:GetService("Players").LocalPlayer.PlayerGui.Islands.Money.RemoteEvent:FireServer(ohNumber1)
                end
            end)
        end
    end
